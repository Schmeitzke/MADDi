...
from data.datasets import GeneticDataset
...
def main(args):
    ...
    # Create datasets
    train_dataset = GeneticDataset(X_train, y_train)
    test_dataset = GeneticDataset(X_test, y_test)
    ...
    # Create model
    model = GeneticModel(input_size=X_train.shape[1])
    model.to(device)
    ...
    # Plot training history
    plot_history(history, save_path=f'genetic_history_lr{args.learning_rate}_bs{args.batch_size}_epochs{args.epochs}.png')
    ...
    cr, precision, recall, thresholds = calc_confusion_matrix(
        test_predictions, test_labels, 'genetic', args.learning_rate, args.batch_size, args.epochs)
    
    # Save model
    torch.save(model.state_dict(), f'genetic_model_lr{args.learning_rate}_bs{args.batch_size}_epochs{args.epochs}.pth')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train genetic model')
    parser.add_argument('--train_features', type=str, default='X_train_vcf.pkl', help='Path to training features')
    parser.add_argument('--train_labels', type=str, default='y_train_vcf.pkl', help='Path to training labels')
    parser.add_argument('--test_features', type=str, default='X_test_vcf.pkl', help='Path to test features')
    parser.add_argument('--test_labels', type=str, default='y_test_vcf.pkl', help='Path to test labels')
